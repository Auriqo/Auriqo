...
Include the smallest useful log excerpt and remove cookies, OAuth tokens, API keys, webhook URLs, device identifiers and personal details. A screenshot or short recording is welcome when it makes the problem easier to understand.

Auriqo has no guaranteed response time. Maintainers may close reports that are duplicates, unsupported by the current alpha build or caused by an unavailable third-party service, while keeping the discussion respectful and useful.

See the [roadmap](ROADMAP.md) for project direction and [CONTRIBUTING.md](CONTRIBUTING.md) for code and documentation changes.
