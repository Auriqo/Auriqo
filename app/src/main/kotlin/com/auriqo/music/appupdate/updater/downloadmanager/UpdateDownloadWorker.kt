package com.auriqo.music.appupdate.updater.downloadmanager
...
class UpdateDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val downloadFile = if (isZip) File(downloadDir, "auriqo_temp.zip") else File(downloadDir, "auriqo.apk")
            ...
            val finalFile = if (isZip) {
                val targetApkFile = File(downloadDir, "auriqo.apk")
                var extracted = false
                try {
